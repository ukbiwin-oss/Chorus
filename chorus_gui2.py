...
# Strip ANSI escape/control junk from terminal output
...
class ChorusApp:
    ...
    def _build_ui(self):
        ...
        # Prompt pane
        prompt_pane = ttk.Frame(self.paned, padding=0)
        ...
        # Output pane
        output_pane = ttk.Frame(self.paned, padding=0)
        ...
        self.paned.add(prompt_pane, weight=1)
        self.paned.add(output_pane, weight=4)
        ...
        self.status = ttk.Label(controls, textvariable=self.status_var)
    ...
    def save_output(self):
        content = "".join(self.output_buffer).strip()
        if not content:
